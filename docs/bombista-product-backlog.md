...
**Bombista and its sibling Pregonero ship as a pair** — both public and MIT-licensed, sharing the timeline v2 contract. B18 is closed as a decision record: go public, **history deliberately left untouched**, master excerpt reachable as an accepted outcome (full record below). B20's step 0 — the pure refactor that lifted `promote`'s merge and the anchor-override parsing out of `cli.py` into `promotion.py` / `songfile.py` / `anchoring.py` — landed via PR #18; it added no tests by design, because `promote`'s existing ones are the proof.

**B19 and B20 are both logged below** (landed via PR #17) — B20 is `bombista serve`, and B19 is absorbed into it as page 2 of that flow rather than built separately. The old output-contract spec was retired in the same change; `docs/timeline-v2-contract.md` is the live contract.
...
| ~~**B14**~~ | ~~Derive and propose the BPM from the aligned onsets~~ — **DROPPED 2026-08-14, and B24 does not reopen it.** See "Why B14 was dropped" below. The tempo is supplied by the performer, where it is exact; B24 gave that typing a control, not a computation. | — |
...
| **B20** | **`bombista serve` — a local web interface.** Full spec: **`docs/bombista-serve-spec.md`** (read it; the table row is a pointer, not a summary). Binds `127.0.0.1` only, runs the existing pipeline in-process, three pages plus the run state: parameters → review and correct → emit. **Absorbs B19.** **Step 0 is a refactor with no feature in it** (§2): `promote`'s merge and the anchor-override parsing live inside `cli.py` as a Click command plus private helpers, so B2's one-merge-path rule is not satisfied and `serve` would have nowhere to import from. Extract first, build second; `promote`'s existing tests are the proof. **Not a hosted service, not a second aligner, not Electron, and not a place tempo gets derived** (rules 4 and 5 — *derived*, and that is unchanged; B24 later made it a place a tempo gets **typed**). | The interface decision below stands as written — this is not its reversal. That decision declined a **GUI** (Electron, a second app, packaging) and pointed at B16 as the cheap capture of most of the value; `serve` is a flag on the existing CLI serving stdlib HTTP, which is the same shape as B16 one step further on. What it adds over B16 is the thing B16 cannot do: the correction loop closes **in the page**, so a REVIEW line is judged and fixed in one place instead of judged in the browser and fixed by retyping a shell command. | L |
...
| ~~**B23**~~ | **`bombista new` and `bombista validate` — the pipeline's front door and its gate** — **DONE 2026-08-24** (Tramoya integration, round A). `new` writes a canonical SP JSON skeleton in the catalogue's key order, with `tempo` and the timing keys **absent rather than scaffolded**, and what it writes already passes `validate`. `validate` has two levels: the default asks *is this file sane* and tolerates work in progress; `--for-performance` asks *is this song finished* and is the gate a song passes before entering a setlist. A **missing** tempo is a warning there; a **partial** one is a failure at both levels. Every problem is listed, never just the first. `--media-dir` is how a logical `media.src` is resolved, because Pregonero keeps that mapping per machine. | Three jobs had no home: supplying a tempo, checking a file is well-formed, and creating one — and all three belong here, because Bombista is already the only thing that writes SP JSON. **Playability is checked in Bombista, not in Pregonero**: every rule lives inside a single song file and needs no gig, and two implementations would be two understandings of the format. A first draft of the design put these checks in Pregonero and was corrected for exactly that reason. The gate found a live defect on its first run over the catalogue — `libertad.json`, 24 lyric lines against a 20-entry timeline. | M |
| ~~**B24**~~ | **The tempo control returns, on `serve`'s page 2** — **DONE 2026-08-24** (round A). Four fields — `bpm`, `numerator`, `denominator`, `countInBars` — a Set, a Clear, and a route that refuses anything short of a whole block. Full reasoning: `docs/bombista-serve-spec.md` **§11.14**, which amends §11.5 rather than replacing it. | **Half of §11.5 expired and half did not, and the distinction is the whole item.** Expired: *tempo is never Bombista's business* — Pregonero loses tempo ownership later in the Tramoya integration, leaving Bombista as the only place a performer can type one in, so the old note pointed at nowhere. Standing: **whole block or nothing** (a bpm-only block NaNs `getBeatsPerBar` — correct scaling, broken pulse, no error anywhere), page 1 keeps its four-row rule and gets no control back, and **B14 stays dropped**: nothing derives, measures or guesses a tempo, and the fields start empty rather than at a plausible `120/4/4`. One gate, `validation.validate_tempo`, shared by the CLI and the route — a second opinion about a valid block is now the failure mode being guarded against. | M |
...
**The Tragedia acceptance QA report stays**, with all 34 rows of Tragedia lines, by the same reasoning that kept the lyric fixtures in `tests/fixtures/`: it is the same already-published lyric text, so removing it buys nothing. Removing it would also destroy a real acceptance record for no gain.
...
1. **No `tempo` block → no Auto mode.** Tempo is a prerequisite, not a nicety. Only 3 of 13 songs have one; the rest are supplied by the performer and filled in by hand, never derived.
...
5. **Bombista's inputs are the audio recordings and the lyrics JSON. Nothing else.** No DAW project files of any kind, no session metadata, no third source of truth. If a fact is not in the audio or the lyrics, it is not Bombista's to know — a human supplies it. This is what keeps the tool generic, offline and portable to the §1 audiences, and it is the boundary the anti-corruption layer in §2 exists to defend. (Established 2026-08-14 after an agent proposed a DAW tempo reader.)
...
This is **not** a reopening of B14, and nothing here licenses deriving the tempo. Rule 5 stands: Bombista's inputs are the audio and the lyrics JSON, nothing else, and no DAW project parsing in any form. The tempo is data entry from what the performer supplies, where it is exact.

**Where that data entry happens moved on 2026-08-24 (B24), and only that.** It is now `serve`'s review page rather than a text editor, because Pregonero loses tempo ownership later in the Tramoya integration. Everything in the paragraphs above survives intact: never derived, whole block or none, absent rather than null.
...
>
> **Out of scope here:** the Pregonero items P1–P4 (separate repo, `projects/pregonero`). They must land before any v2 timeline is loaded into the app — do not promote a migrated song into a running setlist until P3 exists, or it will fail silently.
