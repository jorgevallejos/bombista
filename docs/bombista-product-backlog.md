...
> **⚠ The B19 and B20 rows are not in this file yet.** They are written and waiting in **open PR #17** (`docs/b20-serve-spec`), which logs B20, records B19's absorption into it, and retires the v1 output contract. Code on `main` already cites "B20 §2" in `promotion.py`, `songfile.py`, `anchoring.py` and `CLAUDE.md`, which makes it easy to assume the backlog entry landed with it — it did not. **Merge #17 before adding any B19/B20 text here**, or the same rows get written twice and conflict. **The merge itself is clean** — dry-run verified 2026-08-15 against this commit: its two hunks land at the B14/B16 table boundary and the interface-decision section, clear of everything added here. Although the branch predates step 0, it does **not** revert step 0's documentation: `songfile.py` and `promotion.py` survive in the merged `CLAUDE.md`, and the branch's own change (retiring `output-contract.md` in favour of `timeline-v2-contract.md`) applies on top.
