...
| **B21** | **A typed start time on page 2, with the bound made visible.** The stepper is calibrated on pimiento's 1.22 s error (24 presses); Luz y Sal produced a **47 s** one — ~940 presses, ~42 s of continuous hold. A typed field with the value clamped to the neighbouring lines and rounded to the grid (never coarser than 0.07 s). ⚠ **Open, Jorge's call:** he proposed **dropping the arrows**; the counter-case is that finding an onset is done by ear, and by-ear work is nudge-and-listen, which a text field cannot do. *Type to arrive, nudge to land* keeps both. **Whichever wins, a typed control needs the bound stated** — you feel a stepper stop, you do not feel a text field clamp. Full reasoning: `docs/bombista-serve-spec.md` §12.1. | Found by using it, not by building it: the fixture measured the error Bombista makes **often**, and the interface also has to survive the one it makes **rarely**. Nothing in §6, §8.8 or the synthetic fixture contains a case of that size, so nothing in the design was ever tested against one. | S |
| **B22** | **Close the loop into the vault — promote from page 3, and stop losing two keys.** `promote` writes `ENVELOPE_KEYS` = `timelineVersion`, `leadIn`, `timeline` — **three**. §10.2 gives Bombista **five**, so `linesHash` and `timelineSignedOff` never reach `songs/*.json` (`songs/pimiento.json` confirms it). **Small fix:** a second key set, written when the candidate carries them — they are review facts, not translator requirements, so they must not join `ENVELOPE_KEYS` and inherit its written-as-a-unit refusal. **Right fix:** page 3 promotes, showing the diff before it writes. Also drop the `.sp.json` **extension** — §10.2 settled that the SP JSON *is* the `songs/*.json` format, and a distinct extension reintroduces the distinction that correction removed. §12.2. | `timelineSignedOff` exists **for exactly the journey that drops it** — §10.2's reason for it is that a reviewed timeline stays distinguishable from a machine one *after the file leaves the folder its report is in*, and promote is that departure. `linesHash` is the same loss: promote reads it as a guard and does not write it, so the file it produces cannot guard the next promotion. And §1 says the correction loop closes **in the page**; today it stops at the Downloads folder and the last step is a shell command the page never names. | M |
